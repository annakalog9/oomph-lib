...
//////////////////////TRIANGLE VERSION OF ELEMENTS////////////////////////


//======================class definition==============================
/// A class that solves the Boussinesq approximation of the Navier--Stokes
/// and energy equations by coupling two pre-existing classes. 
/// The TAdvectionDiffusionReactionElement with 
/// bi-quadratic interpolation for the
/// scalar variables (temperature and concentration) and
/// TCrouzeixRaviartElement which solves the Navier--Stokes equations
/// using bi-quadratic interpolation for the velocities and a discontinuous
/// bi-linear interpolation for the pressure. Note that we are free to 
/// choose the order in which we store the variables at the nodes. In this
/// case we choose to store the variables in the order fluid velocities
...
//=========================================================================
template<unsigned DIM>
class DoubleBuoyantTCrouzeixRaviartElement :
 public virtual TAdvectionDiffusionReactionElement<2,DIM,3>,
 public virtual TCrouzeixRaviartElement<DIM>
{
...
public:
 ...
 /// initialise the pointer to the Rayleigh number to point
 /// to the default value of 0.0.
 DoubleBuoyantTCrouzeixRaviartElement() : 
  TAdvectionDiffusionReactionElement<2,DIM,3>(),
 ...
 /// generic for any multi-physics element of this type.
 unsigned required_nvalue(const unsigned &n) const
  {return (TAdvectionDiffusionReactionElement<2,DIM,3>::required_nvalue(n) +
           TCrouzeixRaviartElement<DIM>::required_nvalue(n));}
 ...
 void enable_ALE() 
  ...
  //Compute norm overload to NS version
  void compute_norm(double &norm)
  {TCrouzeixRaviartElement<DIM>::compute_norm(norm);}
 ...
 /// The recovery order is that of the NavierStokes elements.
 unsigned nrecovery_order() 
  {return TCrouzeixRaviartElement<DIM>::nrecovery_order();}

 /// The number of Z2 flux terms is the same as that in 
 /// the fluid element plus that in the advection-diffusion element
 unsigned num_Z2_flux_terms()
  {
   return (TCrouzeixRaviartElement<DIM>::num_Z2_flux_terms() +
           TAdvectionDiffusionReactionElement<2,DIM,3>::num_Z2_flux_terms());
  }
 ...
 void get_Z2_flux(const Vector<double>& s, Vector<double>& flux)
  {
   //Find the number of fluid fluxes
   unsigned n_fluid_flux = 
    TCrouzeixRaviartElement<DIM>::num_Z2_flux_terms();

   //Fill in the first flux entries as the velocity entries
   TCrouzeixRaviartElement<DIM>::get_Z2_flux(s,flux);
...
//=========================================================================
template<>
double DoubleBuoyantTCrouzeixRaviartElement<2>::Default_Physical_Constant_Value=1.0;
...
//=======================================================================
template<unsigned int DIM>
class FaceGeometry<DoubleBuoyantTCrouzeixRaviartElement<DIM> >: 
...
//=======================================================================
template<>
class FaceGeometry<FaceGeometry<DoubleBuoyantTCrouzeixRaviartElement<2> > >: 
public virtual PointElement
{
  public:
 FaceGeometry() : PointElement() {}
  ...
  public:
    ...
    Vector<std::pair<Data*, unsigned>> data_values_of_field(const unsigned& fld)
    {
      ...
      //Otherwise we have the advected temperature and solute components, which are stored
      //after the velocities. The pressure is not stored at the nodes, so we must subtract
      //from the index
      else
       {
        // Loop over all nodes
        ...
        {
          // Add the data value associated with the velocity components
          data_values.push_back(std::make_pair(this->node_pt(n), fld-1));
        }
    ...
    /// Number of fields to be projected: dim+1+3, corresponding to
    /// velocity components and  pressure and the two advected components
                     ...
                     const Vector<double>& s)
    {
      ...
      else if (fld < n_dim)
      {
       ...
       {
        return this->interpolated_c_adv_diff_react(t,s,fld-(n_dim+1));
       }
    }
    ...
    int local_equation(const unsigned& fld, const unsigned& j)
    {
     const unsigned n_dim = this->dim();
      ...
      else if(fld < n_dim)
      {
       ...
       {
        const unsigned c_nodal_index = this->c_index_adv_diff_react(fld-(n_dim+1));
  ...
  class FaceGeometry<ProjectableDoubleBuoyantElement<ELEMENT>>
  ...
  //=======================================================================
  template<class ELEMENT>
  class FaceGeometry<FaceGeometry<ProjectableDoubleBuoyantElement<ELEMENT>>>
